...
const Navbar: React.FC<NavbarProps> = ({ user, onLogout }) => {
  ...
  return (
    <nav className="bg-xpm-blue text-white sticky top-0 z-50 shadow-lg border-b border-white/5">
      ...
      {isOpen && (
        <div className="md:hidden bg-xpm-dark border-t border-white/5 px-2 pt-2 pb-3 space-y-1 shadow-2xl">
          {navLinks.map((link) => (
            (!link.private || user) && (
              <Link
                key={link.path}
          ...
          {user ? (
            ...
            <div className="pt-4 space-y-2 px-2">
              <Link to="/login" onClick={() => setIsOpen(false)} className="block px-4 py-3 rounded-lg text-base font-bold text-center border border-white/10">Login</Link>
              <Link to="/signup" onClick={() => setIsOpen(false)} className="block px-4 py-3 rounded-lg text-base font-bold text-center bg-white/10">Sign Up</Link>
            </div>
          )}
        </div>
      )}
